...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-xl bg-cana-cream/60 p-4">
      <div className="mb-3 text-[11px] font-semibold uppercase tracking-widest text-cana/70">
        {title}
      </div>
      {children}
    </div>
...
export default function ProfileCardItem({
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-cana-rule bg-white shadow-sm">
      ...
      {open && (
        <div className="space-y-3 border-t border-cana-rule px-5 py-4">
              ...
              <div className="mt-3 space-y-2">
                {!!p.hobbies?.length && (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {p.hobbies.map((h) => (
                        <span key={h} className="rounded-full bg-white px-2.5 py-1 text-xs text-cana-ink3">{h}</span>
                      ))}
                    </div>
                  </div>
                )}
                {!!p.personality?.length && (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {p.personality.map((t) => (
                        <span key={t} className="rounded-full bg-white px-2.5 py-1 text-xs text-cana-ink3">{t}</span>
                      ))}
                    </div>
                  </div>
                )}
